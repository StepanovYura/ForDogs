...
function client() {
  ...
  return new S3Client({
    ...
    // Свежие версии AWS SDK по умолчанию вписывают в подписанную ссылку
    // контрольную сумму тела запроса (x-amz-checksum-crc32). Для ссылки,
    // по которой файл потом заливает браузер, сумма считается от пустого
    // тела и с настоящим файлом не сходится — хранилище такой запрос
    // отклоняет. Просим считать сумму только когда это обязательно.
